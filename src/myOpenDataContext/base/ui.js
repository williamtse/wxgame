...
export class ListItem extends Element{
    ...
    onDrag(x, y) {
        if(this.dragable){
            let lastItem = this.listInstance.items[this.items.length-1]
            let firstItem = this.listInstance.items[0]
            if( (this.listInstance.h+this.listInstance.y < lastItem.y+lastItem.h+10 && y<0) //向上滑动仍有空间
                ||(this.listInstance.y > firstItem.y && y>0)  //向下滑动仍有空间
            ){
                this.listInstance.items.forEach(function (it) {
                    it.y+=y
                })
            }
        }
